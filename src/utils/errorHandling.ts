...
export function categorizeError(error: unknown): ErrorInfo {
  ...
  if (errorMessage.includes('JSON') || errorMessage.includes('invalid JSON')) {
    return {
      ...
      suggestion: 'Please check your JSON file for syntax errors like missing commas, quotes, or brackets.',
      canRetry: true
    };
  }
  ...
  if (errorMessage.includes('XML') || errorMessage.includes('malformed XML')) {
    return {
      ...
      suggestion: 'Please ensure your XML file is well-formed with proper tags and structure.',
      canRetry: true
    };
  }
  ...
  if (errorMessage.includes('CSV') || errorMessage.includes('malformed CSV')) {
    return {
      ...
      suggestion: 'Please check your CSV file for proper comma separation and consistent columns.',
      canRetry: true
    };
  }
  ...
  if (errorMessage.includes('empty') || errorMessage.includes('size === 0')) {
    return {
      ...
      suggestion: 'The input file may be corrupted or in an unsupported format. Try a different file.',
      canRetry: true
    };
  }
  ...
  if (errorMessage.includes('not supported') || errorMessage.includes('unsupported')) {
    return {
      ...
      suggestion: 'This conversion combination is not currently supported. Try a different output format.',
      canRetry: false
    };
  }
  ...
  if (errorMessage.includes('memory') || errorMessage.includes('out of memory')) {
    return {
      ...
      suggestion: 'The file is too large. Try with a smaller file or different format.',
      canRetry: false
    };
  }
  ...
  return {
    ...
    suggestion: 'Please try again. If the problem persists, try a different file or format.',
    canRetry: true
  };
}